...
class Adam(Optimizer):
    ...
    def __init__(self, seq, lr=0.01, betas=(0.9, 0.999), eps=10e-8):
        super().__init__(seq)
        self._lr = lr
        self._betas = betas
        self._eps = eps
    ...
    def update(self, parameter, grad_parameter, layer_id, weight_type):
        ...
        s = beta2 * s + (1 - beta2) * grad_parameter * grad_parameter
        n_it += 1
        self.set_state(key, (v, s, n_it))
        v_corrected = v / (1 - beta1 ** n_it)
        s_corrected = s / (1 - beta2 ** n_it)
        new_parameter = parameter + self._lr * (v_corrected / np.sqrt(s_corrected) + self._eps)
        return new_parameter
